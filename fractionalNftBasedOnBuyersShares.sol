...
contract FractionalNft is Pausable, ERC721, Ownable, ReentrancyGuard{
    ...
    function executeTransaction(
        uint _txIndex, uint256 _tokenId, address _to
      ...
      uint256 smartContractFees = (pricePerShare * 2 / 100); 
      for (uint i = 0; i < idToNFT[_tokenId].fractionalBuyer.length; i++) {
        address payable fractionalBuyer = payable(idToNFT[_tokenId].fractionalBuyer[i]);
        uint256 priceForFractionalOwnersTotalShares = (pricePerShare - smartContractFees) * fractionalOwnersShares[_tokenId][fractionalBuyer];
